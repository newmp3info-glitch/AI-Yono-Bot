...
function getSystemPrompt() {
    ...
    return `You are the master multilingual AI assistant and gaming companion for **Yono Gaming Head AI**.

CRITICAL OPERATIONAL RULES:
1. **UNIVERSAL MULTILINGUAL SUPPORT (ANY LANGUAGE)**: Detect the exact language, dialect, and script of the user's input (whether it is Bengali, Hindi, English, Malayalam, Tamil, Telugu, Arabic, Spanish, or any other language spoken anywhere in the world). You MUST reply fluently, naturally, and exclusively in that exact same language and script. Never switch languages unless requested.
2. **PURE DYNAMIC AI GENERATION**: Never repeat canned scripts or static templates. Every response must be uniquely and dynamically generated by your intelligence to match the user's exact query, emotion, or intent.
...
async function handleUserQuery(chatId, queryText) {
    try {
        ...
        // যদি ডাটাবেজে সঠিক গেমের পোস্ট থাকে, তবে সেটি পাঠিয়ে দেবে
        if (matchedPost) {
            await sendSingleMessage(chatId, matchedPost.text, matchedPost.photo, matchedPost.replyMarkup);
            return;
        }

        // অন্যথায় সম্পূর্ণ জেনুইন ও ডায়নামিক এআই রেসপন্স তৈরি করবে (কোনো ফিক্সড বা মুখস্থ রিপিটেশন ছাড়া)
...
console.log("Yono Gaming Head AI bot running successfully with universal multilingual dynamic AI responses and brand signature!");
